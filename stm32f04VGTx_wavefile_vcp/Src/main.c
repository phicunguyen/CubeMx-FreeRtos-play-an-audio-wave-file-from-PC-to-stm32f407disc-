...
uint8_t info [44];
SemaphoreHandle_t semaVCPReady;
/* USER CODE END PD */
...
void HeartBeatTask(void *arg) {
	//xSemaphoreTake(semaVCPReady, portMAX_DELAY);
	//xSemaphoreTake(semaVCPReady, portMAX_DELAY);
	while (true) {
		HAL_Delay(200);
...
int main(void)
{
  ...
  /* add queues, ... */
  semaVCPReady = xSemaphoreCreateBinary();
  ...
  /* add threads, ... */
  xTaskCreate(HeartBeatTask, "heart beat", 128, 0, 2, 0);
